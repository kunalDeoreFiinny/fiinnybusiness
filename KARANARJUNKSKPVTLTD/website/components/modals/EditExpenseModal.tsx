...
interface EditExpenseModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (expense: ExpenseItem) => Promise<void>;
...
export default function EditExpenseModal({
    ...
    useEffect(() => {
        if (isOpen && expense) {
            setTitle(expense.label || expense.category || "");
            setAmount(expense.amount.toString());
            setPayerId(expense.payerId);
            setSelectedFriendIds(expense.friendIds || []);
        }
    }, [isOpen, expense]);

    const handleSubmit = async () => {
        if (!title || !amount || !expense) return;
        ...
        try {
            ...
            // Simple equal split logic for now (maintaining simplified web editing)
            // TODO: In future, support preserving custom splits if participants haven't changed
            const splitAmount = numAmount / uniqueParticipants.length;
            ...
            const updatedExpense: ExpenseItem = {
                ...
                customSplits,
                // Ensure date is preserved or updated if we add date picker
                // date: expense.date 
            };
