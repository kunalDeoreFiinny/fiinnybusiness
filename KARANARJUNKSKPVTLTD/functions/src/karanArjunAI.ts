...
// Production domains — only these origins can call the function
...
export const karanArjunAIChat = onRequest(
  {
    ...
    timeoutSeconds: 30,
    memory: "256MiB",
    // No region override — defaults to us-central1 which org policy allows
  },
  async (req, res) => {
    ...
    if (uid) {
      ...
      if (currentCount >= MAX_DAILY_CALLS) {
        res.status(429).json({ error: `Daily AI limit reached (${MAX_DAILY_CALLS} calls/day). Try again tomorrow.` });
        return;
      }
...
Answer questions based on this live data. Be specific with numbers. Use ₹ for currency. Keep responses under 300 words. Use markdown formatting (bold, bullets). Always end with one actionable tip.`;
    ...
    const response = await new Promise<{ status: number; data: string }>((resolve, reject) => {
      const reqOut = https.request(options, (resIn) => {
        ...
        resIn.on("end", () => resolve({ status: resIn.statusCode || 500, data }));
      });
    ...
    if (response.status !== 200) { res.status(response.status).json({ error: "OpenAI error" }); return; }
