...
const SAAS_PLANS: Record<string, Record<string, number>> = {
 ...
 */
export const createSaaSOrder = onCall(
  { region: "asia-south1" },
  async (request) => {
 ...
 */
export const verifySaaSPayment = onCall(
  { region: "asia-south1" },
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "Login required.");
    ...
    if (!secret) throw new HttpsError("failed-precondition", "Payment provider not configured.");
 ...
 */
export const getSaaSSubscription = onCall(
  { region: "asia-south1" },
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "Login required.");
